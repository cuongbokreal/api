    ...
    const loadingTextMessages = [
        ...
        'Thiên thần tình yêu đang đến...',
        'Đang kết nối phép thuật...',
        'Sắp xong rồi nè...'
    ];
    ...
    // ==========================================
    // 1. CẤU HÌNH (LẤY TỪ PARAM)
    // ==========================================
    
    // Đọc tham số từ URL
    const urlParams = new URLSearchParams(window.location.search);
    
    const YOUTUBE_VIDEO_ID = urlParams.get('yt') || "IcNbh3T_tEI"; // Mặc định nhạc giáng sinh
    const YOUTUBE_START_SECONDS = parseInt(urlParams.get('start')) || 38; // Mặc định giây 38
    ...
    const CONFIG = {
        ...
        treeHeight: 65,
        treeBaseRadius: 30,
        // Lấy text từ param, nếu không có thì dùng mặc định
        titleText: urlParams.get('title') || "MERRY CHRISTMAS",
        loveText: urlParams.get('love') || "I LOVE YOU ❤️"
    };
    ...
    function onPlayerReady(event) {
        ...
        // Cố gắng phát ngay khi ready nếu hệ thống đã start
        forcePlayMusic();
    }
    ...
    function forcePlayMusic() {
        if (player && typeof player.playVideo === 'function') {
            try {
                ...
                // Nếu bị chặn, người dùng sẽ phải bấm nút loa
            }
        }
    }
    ...
    window.addEventListener('load', () => {
        // Đợi 1 chút cho mọi thứ ổn định rồi start
        setTimeout(() => {
            startSystem();
        }, 500);
        
        // Fallback: Nếu sau 15s mà camera chưa lên thì vẫn ẩn loading để người dùng biết
        setTimeout(() => {
            hideLoading();
        }, 15000);
    });
    ...
    async function startSystem() {
        ...
        hands.onResults(results => {
            // Khi nhận được frame đầu tiên từ camera -> Ẩn Loading
        ...
        // Start camera - Browser will ask for permission here
        ...
        if (state === 'TREE') {
            ...
            photoMeshes.forEach((mesh, i) => {
                if (i === selectedIndex) {
                    ...
                    mesh.scale.lerp(_tempVec3.set(5, 5, 5), 0.1);
